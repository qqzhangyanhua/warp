    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let close_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("", NakedTheme)
                ...
                .with_tooltip(tr_cached(Message::CommonClose))
...
impl NotificationMailboxView {
    fn render_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let label = appearance
            .ui_builder()
            .wrappable_text(
                tr_cached(Message::WorkspaceNotifications).to_string(),
                false,
            )
    ...
    fn render_empty_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            appearance
                .ui_builder()
                .wrappable_text(
                    tr_cached(Message::AgentNotificationNoNotifications).to_string(),
                    false,
                )
                .with_style(UiComponentStyles {
                    font_size: Some(14.),
