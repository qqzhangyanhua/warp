...
fn localized_tip_description(ctx: &AppContext, en: &str) -> String {
    ...
    let message = match en {
        "`/` to open the slash-command menu and access quick agent actions." => Some(Message::AgentTipOpenSlashMenu),
        "<keybinding> to toggle natural language detection and switch between agent and terminal input." => Some(Message::AgentTipToggleNaturalLanguage),
        "`/plan` <prompt> to create a plan for the agent before executing." => Some(Message::AgentTipPlanBeforeExecute),
        "<keybinding> to open the Command Palette and access ZYH actions and shortcuts." => Some(Message::AgentTipOpenCommandPalette),
        "Store reusable workflows, notebooks, and prompts in your" => Some(Message::AgentTipStoreInDrive),
        "Enter a new prompt to redirect the agent while it's running." => Some(Message::AgentTipRedirectRunningAgent),
        "`@` to add context from files, blocks, or ZYH Drive objects to your prompt." => Some(Message::AgentTipAtContext),
        "<keybinding> to attach the prior command output as agent context." => Some(Message::AgentTipAttachPriorOutput),
        "`/init` to index the repo so the agent can understand your codebase." => Some(Message::AgentTipInitIndex),
        "Add agent profiles to customize permissions and models per session." => Some(Message::AgentTipAddProfiles),
        "Right-click a block to fork the conversation from that point." => Some(Message::AgentTipForkFromBlock),
        "Right-click a block to copy a conversation's output." => Some(Message::AgentTipCopyBlockOutput),
        "Drag an image into the pane to attach it as agent context." => Some(Message::AgentTipDragImageContext),
        "Prompt the agent to control interactive tools like node, python, postgres, gdb, or vim." => Some(Message::AgentTipControlInteractiveTools),
        "<keybinding> to open the code review panel and review the agent's changes." => Some(Message::AgentTipOpenCodeReview),
        "`/add-mcp` to add an MCP server to your workspace." => Some(Message::AgentTipAddMcp),
        "`/open-mcp-servers` to view and share MCP servers with your team." => Some(Message::AgentTipOpenMcpServers),
        "`/create-environment` to turn a repo into a remote docker environment an agent can run in." => Some(Message::AgentTipCreateEnvironment),
        "`/add-prompt` to create a reusable prompt for repeatable workflows." => Some(Message::AgentTipAddPrompt),
        "`/add-rule` to create a global agent rule." => Some(Message::AgentTipAddRule),
        "`/fork` to create a fresh copy of the current conversation, optionally with a new prompt." => Some(Message::AgentTipFork),
        "`/open-code-review` to open the code review panel and inspect agent-generated diffs." => Some(Message::AgentTipOpenCodeReviewSlash),
        "`/new` to start a new agent conversation with clean context." => Some(Message::AgentTipNewConversation),
        "`/compact` to summarize the current conversation and free up space in the context window." => Some(Message::AgentTipCompact),
        "`/usage` to show your current AI credits usage." => Some(Message::AgentTipUsage),
        "Use the `oz` command to run an Oz agent in headless mode, useful for remote machines." => Some(Message::AgentTipOzHeadless),
        "Right-click selected text to attach it as agent context." => Some(Message::AgentTipAttachSelectedText),
        "Use `AGENTS.md` or `CLAUDE.md` to apply project-scoped rules." => Some(Message::AgentTipProjectRulesFiles),
        "Paste a URL to attach that webpage as context for the agent." => Some(Message::AgentTipPasteUrlContext),
        "Warpify a remote SSH session to enable Oz inside that environment." => Some(Message::AgentTipWarpifySsh),
        "Switch agent profiles to quickly change models and agent permissions." => Some(Message::AgentTipSwitchProfiles),
        "`/init` to generate a `WARP.md` file and define project rules for the agent." => Some(Message::AgentTipInitWarpMd),
        "<keybinding> to auto-approve the agent's commands and diffs for the rest of the session." => Some(Message::AgentTipAutoApprove),
        "Type `&` or use the handoff chip to move a local conversation to the cloud." => Some(Message::AgentTipHandoffCloud),
        "Enable desktop notifications to get an alert when an agent needs your attention." => Some(Message::AgentTipDesktopNotifications),
        "<keybinding> to cancel the current agent task." => Some(Message::AgentTipCancelTask),
        "Hold <keybinding> to speak your prompt directly to the agent." => Some(Message::AgentTipVoicePrompt),
        _ => None,
    };
    if let Some(message) = message {
        crate::i18n::tr(ctx, message).to_string()
    } else {
        en.to_string()
    }
}
