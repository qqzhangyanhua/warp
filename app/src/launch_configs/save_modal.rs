...
impl LaunchConfigSaveModal {
    ...
    fn render_save_config_button(
        ...
        self.save_modal_button(
            appearance,
            tr_cached(Message::LaunchSaveConfiguration).to_owned(),
    ...
    fn render_open_file_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        self.save_modal_button(
            appearance,
            tr_cached(Message::LaunchOpenYamlFile).to_owned(),
        ...
        let header = Flex::row()
            .with_child(
                Shrinkable::new(
                    ...
                    Align::new(
                        Text::new_inline(
                            tr_cached(Message::LaunchSaveCurrentConfiguration),
        ...
        let link_to_docs = Container::new(
            ConstrainedBox::new(
                appearance
                    .ui_builder()
                    .link(
                        tr_cached(Message::LaunchLinkDocumentation).to_string(),
                        Some(
        ...
        let info = match &self.save_state {
                    ...
                    self.render_formatted_text_line(
                        ...
                        vec![
                            FormattedTextFragment::plain_text(tr_cached(
                                Message::LaunchSavedSuccessfullyTo,
                            )),
                ...
                self.render_text_block(
                    ...
                    match failure_type {
                        FailureType::FileAlreadyExists => {
                            tr_cached(Message::LaunchSaveNameExists).to_string()
                        }
                        FailureType::Other => tr_cached(Message::LaunchSaveIssue).to_string(),
                    },
                )
                .with_padding_bottom(24.)
                .finish(),
            ...
            SaveState::NotSaved => {
                let text = if self.open_modal_keybinding_str.is_empty() {
                    tr_cached(Message::LaunchSaveDescription).to_string()
                } else {
                    tr_cached(Message::LaunchSaveDescriptionWithShortcut)
                        .replace("{}", &self.open_modal_keybinding_str)
                };
                header
                    .with_child(
                        self.render_formatted_text_line(
...
impl View for LaunchConfigSaveModal {
    ...
    fn accessibility_contents(&self, _ctx: &AppContext) -> Option<AccessibilityContent> {
        Some(AccessibilityContent::new(
            tr_cached(Message::LaunchSaveConfiguration),
            tr_cached(Message::A11yLaunchSaveConfigModalHelp),
