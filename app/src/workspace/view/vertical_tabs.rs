...
fn render_groups(
    ...
    if workspace.tabs.is_empty() {
        return Container::new(
            Text::new_inline(
                tr(app, Message::WorkspaceNoTabsOpen),
    ...
    if visible_tabs.is_empty() {
        if query.is_empty() {
            ...
            return Container::new(
                Text::new_inline(
                    tr_cached(Message::WorkspaceNoTabsMatchSearch),
                    appearance.ui_font_family(),
                    12.,
                )
                .with_color(theme.sub_text_color(theme.background()).into())
...
pub(super) fn render_settings_popup(
    ...
    let view_as_header = Container::new(
        Text::new_inline(
            tr_cached(Message::WorkspaceViewAs).to_string(),
            appearance.ui_font_family(),
    ...
    let tab_item_header = Container::new(
        Text::new_inline(
            tr_cached(Message::WorkspaceTabItem).to_string(),
            appearance.ui_font_family(),
            SETTINGS_POPUP_MENU_ITEM_FONT_SIZE,
        )
        .with_color(sub_text.into())
    ...
    if show_focused_session_controls {
        ...
        if matches!(current_mode, VerticalTabsViewMode::Compact) {
            ...
            let subtitle_header = Container::new(
                Text::new_inline(
                    tr_cached(Message::WorkspaceAdditionalMetadata).to_string(),
                    appearance.ui_font_family(),
                    SETTINGS_POPUP_MENU_ITEM_FONT_SIZE,
                )
                .with_color(sub_text.into())
