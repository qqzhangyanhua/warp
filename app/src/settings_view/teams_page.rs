...
impl std::fmt::Display for TeamsInviteOption {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            "{}",
            match self {
                TeamsInviteOption::Link => tr_cached(Message::TeamsInviteOptionLink),
                TeamsInviteOption::Email => tr_cached(Message::TeamsInviteOptionEmail),
            },
        )
    }
}
...
impl TeamsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<TeamsPageView>) -> Self {
        ...
        let create_team_editor = Self::editor(
            ...
            tr_cached(Message::TeamsTeamNamePlaceholder),
            font_size,
            ctx,
        );
        ...
        let approve_domains_block_editor = ctx.add_typed_action_view(|ctx| {
            WordBlockEditorView::new(
                ctx,
                tr_cached(Message::TeamsDomainsCommaSeparated),
                font_size,
                vec![',', ' '],
                MAX_CHIP_WIDTH,
        ...
        let email_invites_block_editor = ctx.add_typed_action_view(|ctx| {
            WordBlockEditorView::new(
                ctx,
                tr_cached(Message::TeamsEmailsCommaSeparated),
                font_size,
                vec![',', ' '],
                MAX_CHIP_WIDTH,
        ...
        let rename_team_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(tr_cached(Message::TeamsYourNewTeamName), ctx);
            input
        });
        ...
        let transfer_ownership_modal = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(tr_cached(Message::TeamsTransferOwnershipQuestion).to_string()),
    ...
    fn handle_model_event(
        ...
        match event {
            ...
            UserWorkspacesEvent::EmailInviteRejected(err) => {
                self.update_team_members_state(ctx);
                self.show_error(tr_cached(Message::TeamsFailedSendInvite), Some(err), ctx)
            }
            ...
            UserWorkspacesEvent::ToggleInviteLinksSuccess => {
                self.show_success(tr_cached(Message::TeamsToggledInviteLinks), ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::ToggleInviteLinksRejected(err) => {
                self.show_error(
                    tr_cached(Message::TeamsFailedToggleInviteLinks),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::ResetInviteLinks => {
                self.show_success(tr_cached(Message::TeamsResetInviteLinks), ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::ResetInviteLinksRejected(err) => {
                self.show_error(
                    tr_cached(Message::TeamsFailedResetInviteLinks),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::DeleteTeamInvite => {
                self.update_team_members_state(ctx);
                self.show_success(tr_cached(Message::TeamsDeletedInvite), ctx);
            }
            UserWorkspacesEvent::DeleteTeamInviteRejected(err) => {
                self.show_error(tr_cached(Message::TeamsFailedDeleteInvite), Some(err), ctx);
            }
            ...
            UserWorkspacesEvent::AddDomainRestrictionsRejected(err) => {
                self.show_error(tr_cached(Message::TeamsFailedAddDomain), Some(err), ctx)
            }
            UserWorkspacesEvent::DeleteDomainRestrictionSuccess => {
                self.update_approved_domains_state(ctx);
            }
            UserWorkspacesEvent::DeleteDomainRestrictionRejected(err) => {
                self.show_error(tr_cached(Message::TeamsFailedDeleteDomain), Some(err), ctx)
            }
            ...
            UserWorkspacesEvent::GenerateUpgradeLinkRejected(err) => {
                self.show_error(tr_cached(Message::TeamsFailedUpgradeLink), Some(err), ctx)
            }
            ...
            UserWorkspacesEvent::GenerateStripeBillingPortalLinkRejected(err) => {
                self.show_error(tr_cached(Message::TeamsFailedBillingLink), Some(err), ctx)
            }
            UserWorkspacesEvent::ToggleTeamDiscoverabilitySuccess => {
                self.show_success(tr_cached(Message::TeamsToggledDiscoverability), ctx);
                ctx.notify();
            }
            UserWorkspacesEvent::ToggleTeamDiscoverabilityRejected(err) => {
                self.show_error(
                    tr_cached(Message::TeamsFailedToggleDiscoverability),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::JoinTeamWithTeamDiscoverySuccess => {
                ...
                let message = self.user_workspaces.as_ref(ctx).current_team().map_or(
                    tr_cached(Message::TeamsSuccessfullyJoinedTeam).to_string(),
                    |team| {
                        tr_cached(Message::TeamsSuccessfullyJoinedNamed)
            ...
            UserWorkspacesEvent::JoinTeamWithTeamDiscoveryRejected(err) => {
                self.show_error(tr_cached(Message::TeamsFailedJoinTeam), Some(err), ctx);
            }
            ...
            UserWorkspacesEvent::TransferTeamOwnershipSuccess => {
                self.show_success(
                    tr_cached(Message::TeamsSuccessfullyTransferredOwnership),
                    ctx,
                );
                ctx.notify();
            }
            UserWorkspacesEvent::TransferTeamOwnershipRejected(err) => {
                self.show_error(
                    tr_cached(Message::TeamsFailedTransferOwnership),
                    Some(err),
                    ctx,
                );
            }
            UserWorkspacesEvent::SetTeamMemberRoleSuccess => {
                self.update_team_members_state(ctx);
                self.show_success(tr_cached(Message::TeamsSuccessfullyUpdatedMemberRole), ctx);
            }
            UserWorkspacesEvent::SetTeamMemberRoleRejected(err) => {
                self.show_error(
                    tr_cached(Message::TeamsFailedUpdateMemberRole),
                    Some(err),
                    ctx,
                );
            }
    ...
    fn handle_team_update_event(
        ...
        match event {
            TeamUpdateManagerEvent::LeaveError => {
                let error = tr_cached(Message::TeamsErrorLeavingTeam).to_string();
                self.show_error(error, None, ctx);
            }
            TeamUpdateManagerEvent::LeaveSuccess => {
                self.show_success(tr_cached(Message::TeamsSuccessfullyLeftTeam), ctx);
                ctx.notify();
            }
            TeamUpdateManagerEvent::RenameTeamSuccess => {
                self.show_success(tr_cached(Message::TeamsSuccessfullyRenamedTeam), ctx)
            }
            TeamUpdateManagerEvent::RenameTeamError => {
                self.show_error(tr_cached(Message::TeamsFailedRenameTeam), None, ctx)
            }
        }
    }
    ...
    fn copy_invite_link(&mut self, link: &str, ctx: &mut ViewContext<Self>) {
        ...
        self.show_toast(
            tr_cached(Message::TeamsLinkCopied),
            ToastFlavor::Default,
            ctx,
        );
    }
    ...
    fn add_domain_restrictions(&mut self, team_uid: ServerId, ctx: &mut ViewContext<Self>) {
        ...
        if !invalid_domains.is_empty() {
            let error = tr_cached(Message::TeamsInvalidDomainsWithCount)
        ...
        self.show_success(
            tr_cached(Message::TeamsDomainRestrictionsAdded)
    ...
    fn send_email_invites(&mut self, team_uid: ServerId, ctx: &mut ViewContext<Self>) {
        ...
        if !invalid_emails.is_empty() {
            let error = tr_cached(Message::TeamsInvalidEmailsWithCount)
        ...
        let message = if unique_emails.len() == 1 {
            tr_cached(Message::TeamsInviteOnTheWay).to_string()
        } else {
            tr_cached(Message::TeamsInvitesOnTheWay)
    ...
    fn team_to_item_list(&self, team: &Team, current_user_email: &str) -> Vec<Item> {
        ...
        team.pending_email_invites.iter().for_each(|email_invite| {
            ...
            let actions = if current_user_has_admin_permissions {
                vec![ItemAction {
                    icon: Icon::X,
                    label: tr_cached(Message::TeamsCancelInvite)
                        .to_string()
                        .to_string(),
            ...
            if member.email != *current_user_email {
                ...
                if current_user_has_owner_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::Users,
                        label: tr_cached(Message::TeamsTransferOwnership)
                            .to_string()
                            .to_string(),
                    ...
                    && !team_member_has_owner_permissions
                {
                    if team_member_has_admin_permissions {
                        actions.push(ItemAction {
                            icon: Icon::ArrowDown,
                            label: tr_cached(Message::TeamsDemoteFromAdmin)
                                .to_string()
                                .to_string(),
                        ...
                        actions.push(ItemAction {
                            icon: Icon::ArrowUp,
                            label: tr_cached(Message::TeamsPromoteToAdmin)
                                .to_string()
                                .to_string(),
                            action: TeamsPageAction::SetTeamMemberRole {
                                team_uid: team.uid,
                ...
                if current_user_has_admin_permissions && !team_member_has_owner_permissions {
                    actions.push(ItemAction {
                        icon: Icon::X,
                        label: tr_cached(Message::TeamsRemoveFromTeam)
                            .to_string()
                            .to_string(),
...
impl TeamsWidget {
    ...
    fn render_grow_team_warning_alert(
        ...
        let title = match warning {
            GrowTeamWarning::SeatCapReached => tr_cached(Message::TeamsYourTeamIsFull),
            GrowTeamWarning::SeatCapExceeded => tr_cached(Message::TeamsExceededMemberLimit),
            GrowTeamWarning::PaymentPastDue => tr_cached(Message::TeamsPaymentPastDue),
            GrowTeamWarning::PaymentUnpaid => tr_cached(Message::TeamsSubscriptionUnpaid),
        };
        ...
        let body_prefix = match warning {
            GrowTeamWarning::SeatCapReached => tr_cached(Message::TeamsReachedMemberLimitBody),
            GrowTeamWarning::SeatCapExceeded => tr_cached(Message::TeamsExceededMemberLimitBody),
            GrowTeamWarning::PaymentPastDue => tr_cached(Message::TeamsInvitesRestrictedPastDue),
            GrowTeamWarning::PaymentUnpaid => tr_cached(Message::TeamsInvitesRestrictedUnpaid),
        };
        ...
        let cta_sentence = if !has_admin_permissions {
            if is_delinquency {
                tr_cached(Message::TeamsContactAdminRestoreAccess)
            } else {
                tr_cached(Message::TeamsContactAdminGrowTeam)
            }
        } else {
            match cta {
                GrowTeamWarningCta::Upgrade => tr_cached(Message::TeamsUpgradeToGrow),
                GrowTeamWarningCta::UpdateBilling => tr_cached(Message::TeamsUpdatePaymentRestore),
                GrowTeamWarningCta::ContactSupport => {
                    tr_cached(Message::TeamsContactSupportRestore)
                }
                GrowTeamWarningCta::None => {
                    if is_delinquency {
                        tr_cached(Message::TeamsContactSupportRestore)
                    } else {
                        tr_cached(Message::TeamsContactSalesGrow)
                    }
                }
            }
        };
    ...
    fn outgrow_upgrade_line_copy(
        ...
        if billing_metadata.customer_type == CustomerType::Business {
            (
                ...
                tr_cached(Message::TeamsUpgradeToEnterpriseSuffix),
            )
        } else {
            (
                tr_cached(Message::TeamsUpgradeToBusiness),
                tr_cached(Message::TeamsUpgradeToBusinessSuffix),
            )
        }
    }
    ...
    fn render_team_member_cost_info(
        ...
        let prorated_message = if has_admin_permissions {
            tr_cached(Message::TeamsChargedPortionAdmin)
        } else {
            tr_cached(Message::TeamsChargedPortionNonAdmin)
        };
        ...
        {
            tr_cached(Message::TeamsAdditionalMembersBilledWithRate)
            ...
            tr_cached(Message::TeamsAdditionalMembersBilled)
                .replace("{prorated_message}", prorated_message)
        };
        ...
        match team.billing_metadata.delinquency_status {
            DelinquencyStatus::PastDue => {
                left_side.add_child(
                    Container::new(self.render_delinquency_badge(
                        appearance,
                        tr_cached(Message::TeamsPastDueBadge).into(),
            ...
            DelinquencyStatus::Unpaid => {
                left_side.add_child(
                    Container::new(self.render_delinquency_badge(
                        appearance,
                        tr_cached(Message::TeamsUnpaidBadge).into(),
    ...
    fn render_manage_billing_button(
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    TextAndIconAlignment::IconFirst,
                    tr_cached(Message::TeamsManageBilling),
    ...
    fn render_admin_panel_button(
        ...
        appearance
            ...
            .with_text_and_icon_label(
                TextAndIcon::new(
                    TextAndIconAlignment::IconFirst,
                    tr_cached(Message::TeamsOpenAdminPanel),
            ...
            if team.billing_metadata.can_upgrade_to_higher_tier_plan() {
                let description = if team.billing_metadata.can_upgrade_to_build_plan() {
                    ...
                    match team.billing_metadata.customer_type {
                        CustomerType::Prosumer => tr_cached(Message::TeamsUpgradeToTurbo),
                        CustomerType::Turbo => tr_cached(Message::TeamsUpgradeToLightspeed),
                        _ => tr_cached(Message::TeamsComparePlans),
                    }
                };
                billing_links.add_child(
    ...
    fn render_plan_usage(
        ...
        let sub_header_text = match team.billing_metadata.customer_type {
            CustomerType::Free => tr_cached(Message::TeamsFreePlanUsageLimits),
            _ => tr_cached(Message::TeamsPlanUsageLimits),
        };
        ...
        if let Some(policy) = team.billing_metadata.tier.shared_notebooks_policy {
            if !policy.is_unlimited {
                ...
                shared_notebooks_column.add_child(self.render_plan_usage_header(
                    tr_cached(Message::TeamsSharedNotebooks).into(),
                    appearance,
                ));
        ...
        if let Some(policy) = team.billing_metadata.tier.shared_workflows_policy {
            if !policy.is_unlimited {
                ...
                shared_workflows_column.add_child(self.render_plan_usage_header(
                    tr_cached(Message::TeamsSharedWorkflows).into(),
                    appearance,
                ));
    ...
    fn render_team_invitation_section(
        ...
        invitation_section.add_child(
            Container::new(self.render_subsection_header(
                tr_cached(Message::TeamsInviteTeamMembers).to_owned(),
                appearance,
            ))
    ...
    fn render_invite_by_link_section(
        ...
        let header = self
            .render_subsubsection_header(tr_cached(Message::TeamsByLink).to_owned(), appearance);
        let text_column = if has_admin_permissions {
            Flex::column()
                .with_child(header)
                .with_child(
                    Container::new(self.render_sub_text(
                        tr_cached(Message::TeamsInviteLinkToggleInstructions).into(),
                        appearance,
                ...
                section.add_child(
                    Align::new(
                        appearance
                            .ui_builder()
                            .link(
                                tr_cached(Message::TeamsResetLinks).into(),
                                None,
    ...
    fn render_invite_by_email_section(
        ...
        // tr_cached(Message::TeamsByEmail) subsection header
        section.add_child(
            Container::new(self.render_subsubsection_header(
                tr_cached(Message::TeamsByEmail).to_owned(),
                appearance,
            ))
            .with_padding_top(CONTENT_SEPARATION_PADDING)
        ...
        section.add_child(
            Container::new(self.render_sub_text(
                tr_cached(Message::TeamsInviteByEmailExpiry).into(),
                appearance,
                Some(Coords::uniform(0.).right(48.)),
            ))
            ...
            && view.email_invites_block_editor_state.num_chips > 0
        {
            section.add_child(
                Container::new(self.render_error_sub_text(
                    tr_cached(Message::TeamsInvalidEmails).into(),
                    appearance,
                ))
                .with_padding_top(8.)
                .finish(),
    ...
    fn render_team_members_section(
        ...
        // 1) tr_cached(Message::TeamsTeamMembers) header row
        let header_row = Flex::row()
            .with_main_axis_size(MainAxisSize::Max)
            .with_main_axis_alignment(MainAxisAlignment::SpaceBetween)
            .with_cross_axis_alignment(CrossAxisAlignment::Center)
            .with_child(self.render_subsection_header(
                tr_cached(Message::TeamsTeamMembers).to_owned(),
                appearance,
            ))
    ...
    fn render_team_members_count(&self, team: &Team, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let count_label = if count == 1 {
            tr_cached(Message::TeamsOneTeamMember).to_string()
        } else {
            tr_cached(Message::TeamsNTeamMembers).replace("{count}", &count.to_string())
        };
        ...
        let tooltip_text = tr_cached(Message::TeamsPlanCapacity)
    ...
    fn render_outgrow_upgrade_cta(
        ...
        let prefix = self.render_sub_text(
            tr_cached(Message::TeamsNeedMoreSeats).to_string(),
            appearance,
            None,
        );
        let link = appearance
    ...
    fn render_approved_domains_section(
        ...
        if has_admin_permissions {
            section.add_child(
                Container::new(self.render_sub_text(
                    tr_cached(Message::TeamsInviteLinkDomainRestrictions).into(),
                    appearance,
                    Some(Coords::uniform(0.).right(48.)),
                ))
                .with_padding_top(16.)
                ...
                && view.approve_domains_block_editor_state.num_chips > 0
            {
                section.add_child(
                    Container::new(self.render_error_sub_text(
                        tr_cached(Message::TeamsInvalidDomains).into(),
                        appearance,
                    ))
                    .with_padding_top(8.)
                    .finish(),
        ...
        let domains_as_items: Vec<Item> = team
            ...
            .map(|domain_restriction| {
                let actions = if has_admin_permissions {
                    vec![ItemAction {
                        icon: Icon::X,
                        label: tr_cached(Message::TeamsRemoveDomain).to_string(),
    ...
    fn render_discoverability_toggle_section(
        ...
        // Same layout as the tr_cached(Message::TeamsByLink) header row: text column on the left,
        // toggle on the right.
        let header = self.render_subsubsection_header(
            tr_cached(Message::TeamsByDiscovery).to_owned(),
            appearance,
        );

        let domain = current_user_email.split('@').nth(1).unwrap_or("");
        let team_discoverability_instructions =
            tr_cached(Message::TeamsDiscoverabilityWithDomain).replace("{domain}", domain);
    ...
    fn render_leave_or_delete_team_button(
        ...
        let (label, action) = if is_team_owner {
            (
                tr_cached(Message::TeamsDeleteTeam),
                TeamsPageAction::ShowDeleteTeamConfirmationDialog,
            )
        } else {
            (
                tr_cached(Message::TeamsLeaveTeam),
                TeamsPageAction::ShowLeaveTeamConfirmationDialog,
            )
        };
    ...
    fn render_delete_disabled_help_text(
        ...
        if delete_disabled_reason == TeamDeleteDisabledReason::ActivePaidSubscription {
            let link = appearance
                .ui_builder()
                .link(
                    tr_cached(Message::TeamsManagePlan).into(),
                    None,
                    Some(Box::new(move |ctx| {
                ...
                match item.state {
                    ItemState::Expired => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                tr_cached(Message::TeamsExpiredBadge).into(),
                    ...
                    ItemState::Pending => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                tr_cached(Message::TeamsPendingBadge).into(),
                    ...
                    ItemState::Owner => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                tr_cached(Message::TeamsOwnerBadge).into(),
                    ...
                    ItemState::Admin => {
                        pending_and_close_row.add_child(
                            self.render_state_chip(
                                appearance,
                                tr_cached(Message::TeamsAdminBadge).into(),
                                appearance
                                    .theme()
                                    .background()
    ...
    fn render_copy_link_row(
        ...
        let (link_text, button_enabled) = match &team_metadata.invite_code {
            ...
            None => (tr_cached(Message::TeamsFailedLoadInviteLink).into(), false),
        };
        let theme = appearance.theme();
    ...
    /// (e.g. tr_cached(Message::TeamsByLink) / tr_cached(Message::TeamsByEmail) / tr_cached(Message::TeamsByDiscovery)).
    ...
    fn render_create_team_page(
        ...
        page.add_child(render_sub_header(
            appearance,
            tr_cached(Message::SettingsSectionTeams).to_string(),
            None,
        ));
        page.add_child(self.render_sub_header_with_subtext_color(
            appearance,
            tr_cached(Message::TeamsCreateATeam).to_string(),
        ));
        page.add_child(
            Container::new(self.render_description(
                tr_cached(Message::TeamsCreateTeamDescription).to_string(),
                appearance,
            ))
            .with_padding_top(6.)
            .finish(),
        ...
        if view.auth_state.is_on_work_domain().unwrap_or_default() {
            ...
            let checkbox_row_text = if let Some(domain) = view.auth_state.user_email_domain() {
                ...
                tr_cached(Message::TeamsDiscoverabilitySameDomain).to_string()
            };
        ...
        if !view.discoverable_teams_states.is_empty() {
            ...
            page.add_child(self.render_sub_header_with_subtext_color(
                appearance,
                tr_cached(Message::TeamsOrJoinExisting).to_string(),
            ));
    ...
    fn render_single_team_in_team_discovery(
        ...
        let teammate_string = if team_state.team.num_members == 1 {
            tr_cached(Message::TeamsOneTeammate).to_string()
        } else {
            tr_cached(Message::TeamsNTeammates)
        ...
        single_team.add_child(
            Container::new(self.render_sub_text(
                tr_cached(Message::TeamsJoinThisTeamDescription).to_string(),
                appearance,
                None,
            ))
            .with_padding_top(12.)
    ...
    fn render_create_team_button(
        ...
        let mut button = appearance
            ...
            .with_centered_text_label(tr_cached(Message::TeamsCreate).to_owned())
            .with_style(UiComponentStyles {
                font_color: Some(
                    appearance
                        .theme()
        ...
        if team_state.team.team_accepting_invites {
            ...
            appearance
                ...
                .with_centered_text_label(
                    tr_cached(Message::TeamsContactAdminRequestAccess).to_string(),
                )
                .disabled()
                .build()
                .finish()
...
impl SettingsWidget for TeamsWidget {
    ...
    fn search_terms(&self) -> &str {
        tr_cached(Message::TeamsSectionSearchKeywords)
    }
    ...
    fn render(
        ...
        let content = if NetworkStatus::as_ref(app).is_online() {
            ...
            appearance
                .ui_builder()
                .span(tr_cached(Message::TeamsYouAreOffline).to_string())
                .build()
                .finish()
        };
