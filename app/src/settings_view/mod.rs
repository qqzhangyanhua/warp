...
pub(super) fn render_beta_chip(appearance: &Appearance) -> Box<dyn Element> {
    ...
    Container::new(
        Text::new_inline(
            tr_cached(Message::CommonBetaUpper),
...
impl SettingsSection {
    ...
    pub(crate) fn redirect_unavailable_section(self) -> Self {
        if self.hidden_without_account_and_cloud_services() {
            Self::WarpAgent
        } else {
            self
        }
    ...
    pub(crate) fn command_entrypoint_enabled(self) -> bool {
    ...
    fn initial_page_for_zyh(page: Option<Self>) -> Self {
        match page {
            Some(Self::AI) => Self::WarpAgent,
            Some(Self::Code) => Self::CodeIndexing,
            Some(section) if section.unavailable_in_zyh() => Self::WarpAgent,
            Some(Self::Scripting) if !FeatureFlag::WarpControlCli.is_enabled() => {
                Self::default_for_zyh()
            }
            Some(section) if section.is_subpage() => section,
            other => other.unwrap_or_else(Self::default_for_zyh),
        }
    }
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        let pane_configuration =
            ctx.add_model(|_ctx| PaneConfiguration::new(tr_cached(Message::WorkspaceSettings)));
        ...
        // Environments page
        let environments_page_handle: Option<ViewHandle<EnvironmentsPageView>> = None;
        ...
        nav_items.retain(|item| match item {
            SettingsNavItem::Page(section) => !section.unavailable_in_zyh(),
            SettingsNavItem::Umbrella(umbrella) => umbrella
                .subpages
                .iter()
                .any(|section| !section.unavailable_in_zyh()),
        });
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        let section = match section {
            ...
            unavailable if unavailable.unavailable_in_zyh() => SettingsSection::WarpAgent,
            other => other,
        };
    ...
    fn should_render_page(&self, settings_page: &SettingsPage, app: &AppContext) -> bool {
        if settings_page.section.unavailable_in_zyh() {
            return false;
        }
...
impl BackingView for SettingsView {
    ...
    fn render_header_content(
        ...
        view::HeaderContent::simple(tr_cached(Message::WorkspaceSettings))
    }
