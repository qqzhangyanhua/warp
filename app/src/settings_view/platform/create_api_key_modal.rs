...
impl ApiKeyType {
    fn description(&self) -> &'static str {
        match self {
            ApiKeyType::Personal => {
                "This API key is tied to your user and can make requests against your ZYH account."
            }
            ApiKeyType::Team => {
        ...
        let team_id = if selected_type == ApiKeyType::Team {
            ...
            match workspaces.current_team_uid() {
                ...
                None => {
                    self.request_state = RequestState::Idle;
                    ctx.emit(CreateApiKeyModalEvent::Error {
                        message: tr_cached(Message::UnableCreateTeamApiKey).to_string(),
                    });
                    ctx.notify();
                    return;
                }
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CreateApiKeyModalAction::CreateNewAgent => {
                // External oz.warp.dev agent creation is not opened from settings.
            }
        }
    }
}
