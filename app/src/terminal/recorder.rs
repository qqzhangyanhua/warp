    ...
    fn update_recording_state(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        if should_record && !self.is_recording() {
            if let Some(path) = self.start_recording(ctx) {
                ...
                self.show_toast(
                    tr_cached(Message::PtyRecordingStarted).replace("{}", &display_path),
                    Some(file_path),
                    ctx,
                );
            }
            ...
            self.stop_recording();
            self.show_toast(
                tr_cached(Message::PtyRecordingStopped).replace("{}", &display_path),
    ...
    fn show_toast(
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            ...
            if let Some(path) = recording_path {
                ...
                toast = toast
                    .with_link(
                        ToastLink::new(tr_cached(Message::CommonOpen).to_string())
