//! Local MCP server payloads (transport, templates, secrets redaction).
//!
//! These types intentionally do not depend on cloud ownership, GraphQL, server
//! auth, or hosted-service crates. Secret env/header values use
//! `skip_serializing` so they never enter persisted JSON or network payloads.
...
/// JSON-facing MCP server config (CLI or SSE), as written by users/tools.
...
/// Untagged JSON transport variants used in user-facing MCP config files.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(untagged)]
...
/// Runtime MCP server definition used by the local MCP runtime and UI.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Lifecycle state of a running (or starting) MCP server process/session.
...
/// How an MCP server is reached: child process or SSE HTTP endpoint.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Stdio/CLI MCP server command configuration.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Environment variable whose value must never be serialized to disk or network.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StaticEnvVar {
    pub name: String,
    /// To avoid leaking environment variables, values are not serialized.
    #[serde(skip_serializing, default)]
    pub value: String,
}

/// HTTP header whose value must never be serialized to disk or network.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StaticHeader {
    pub name: String,
    /// To avoid leaking header values (which may contain secrets), values are not serialized.
    #[serde(skip_serializing, default)]
    pub value: String,
}

/// SSE transport configuration for an MCP server.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Handlebars-style JSON template body with variable metadata.
...
/// A single template variable key, optionally constrained to allowed values.
...
/// Optional gallery provenance for a templatable MCP server (legacy field).
...
/// User-editable MCP server stored as a JSON template with variables.
...
pub struct TemplatableMCPServer {
    ...
    #[serde(default)]
    pub version: i64, // defaults to 0 if stored objects have no version
    pub gallery_data: Option<GalleryData>,
}

/// Errors when reconstructing a templatable MCP server from stored JSON.
#[derive(Debug)]
